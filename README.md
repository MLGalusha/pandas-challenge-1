 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "# Create a dataframe only of the category with the most entries(so in this case consumables)\n",
    "filtered_df = df[df['category'] == df['category'].value_counts().idxmax()]\n",
    "\n",
    "# Find the subcategory with most entries in the filtered df of consumables\n",
    "filtered_df['subcategory'].value_counts().idxmax()\n"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    ...
    "top_5_clients = df['client_id'].value_counts().head().index.tolist()\n",
    "top_5_clients"
   ]
  },
  {
   ...
   "source": [
    ...
    "sum(df.loc[df['client_id'] == top_5_clients[0], 'qty'])\n"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "tot_cost = df.groupby('order_id')[['line_price']].sum().reset_index()\n",
    "order_id_list = [2742071, 2173913, 6128929]\n",
    "\n",
    "for id in order_id_list:\n",
    "    price = tot_cost.loc[tot_cost['order_id'] == id, 'line_price'].values[0]\n",
    "    print(f\"Order ID {id} had a total price of ${price:,.2f}\")\n"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "for id in top_5_clients:\n",
    "    price = df.loc[df['client_id'] == id, 'line_price'].sum()\n",
    "    print(f\"{id}: ${price:,.2f}\")\n"
   ]
  },
  {
   ...
   "source": [
    ...
    "top_5 = df[df['client_id'].isin(top_5_clients)]\n",
    "\n",
    "# Sum up all the data for the corresponding client ids in the top 5 list\n",
    "summary = top_5.groupby('client_id')[['qty', 'shipping_price', 'line_price', 'line_cost', 'line_profit']].sum().reset_index()\n",
    "\n",
    "# Sort the data back into the top 5 list\n",
    "summary['client_id'] = pd.Categorical(summary['client_id'], categories=top_5_clients, ordered=True)\n",
    "summary = summary.sort_values('client_id').reset_index(drop=True)\n",
    "\n",
    "summary"
   ]
  },
  {
   ...
   "source": [
    ...
    "\n",
    "summary.columns = ['Client ID', 'Units', 'Shipping (millions)', 'Total Revenue (millions)', 'Total Cost (millions)', 'Total Profit (millions)']\n",
    "\n",
    "def currency_format_millions(x):\n",
    "    return x/1000000\n",
    "\n",
    "for column in summary.columns:\n",
    "    if 'millions' in column:\n",
    "        summary[column] = summary[column].apply(currency_format_millions)\n",
    "    else:\n",
    "        continue\n",
    "\n",
    "summary\n"
  ...
  },
  {
   ...
   "source": [
    ...
    "summary = summary.sort_values(by='Total Profit (millions)', ascending=False)\n",
    "summary"
   ]
  },
  {
   ...
   "outputs": [
    {
     "data": {
      "text/plain": [
       "'\\nThe client with the highest total profit is client ID 24741, generating $36.58\\nmillion in profit from 239,862 units sold, contributing to a total revenue of\\n$82.27 million. The second highest profit was from client ID 38378 with $3.27\\nmillion in profit from 73,667 units sold, leading to a total revenue of $12.91\\nmillion. Overall, the data shows a significant variance in total profit among\\nthe top clients, with client 24741 notably outperforming the others.\\n'"
      ]
     },
     "execution_count": 40,
     "metadata": {},
